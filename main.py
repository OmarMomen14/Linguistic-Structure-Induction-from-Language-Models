...
parser.add_argument(
    ...
    help='type of neural net (structformer, transformer, structformer_in_parser)')
parser.add_argument(
...
parser.add_argument('--front_layers', type=int, default=2, help='number of front layers (for structformer_in_parser)')
parser.add_argument('--rear_layers', type=int, default=6, help='number of rear layers (for structformer_in_parser)')
parser.add_argument('--subword', type=str, help='use subword tokenizer')
parser.add_argument(
...
if args.subword:
  ntokens = ptb_corpus.tokenizer.vocab_size
else:
...
if args.model == 'structformer':
  ...
  model = structformer.Transformer(
      ...
      args.pos_emb,
      pad=pad_token)
elif args.model == 'structformer_in_parser':
  model = structformer_in_parser.StructFormer_In_Parser(
      args.nhid,
      args.nlayers,
      ntokens,
      args.nheads,
...
def train():
  ...
  if args.subword:
    train_data = batchify(
      ptb_corpus.train, args.batch_size, device, pad=pad_token, shuffle=True)
  else:
    train_data = batchify(
...
try:
  ...
  for epoch in range(1, args.epochs + 1):
    ...
    #   print('=' * 89)
    #   print('| TEST 10th EPOCH | test loss {:5.2f} | test ppl {:8.2f} | '
...
if args.test_grammar:
  if args.subword:
    test_phrase_grammar.test_subword(model, ptb_corpus, device)
  else:
    test_phrase_grammar.test(model, ptb_corpus, device)
  print('=' * 89)
